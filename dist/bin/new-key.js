...
import { GraphqlError, createUserApiKey } from "../graphql.js";
...
await runCommand(async () => {
    ...
    if (args.badAccess !== undefined) {
        process.stderr.write(`error: unknown access "${args.badAccess}". Use --access=read or --access=read_write.\n`);
        endWith(2);
    }
    ...
    if (args.install) {
        ...
        if (!args.json) {
            ...
            if (claude.installed) {
                ...
                if (status.installed) {
                    process.stdout.write(`  ${theme.ok("✓")} ${theme.bold("Status line")} ${theme.dim("connection state shows in Claude Code")}\n`);
                }
            }
        }
        const context = {
...
//# sourceMappingURL=new-key.js.map