...
import { MalformedConfigError, readConfig, writeConfig } from "../mcp-config.ts";
...
export const STATUS_COMMAND =
  '_j="$(ls -dt "$HOME/.claude/plugins/cache/jitera/jitera-connect/"*"/dist/bin/statusline.js" 2>/dev/null | head -1)"; ' +
...
export function installStatusLine({
  home,
  dryRun = false,
}: StatusLineInstallOptions): StatusLineInstallResult {
  ...
  const existing = config["statusLine"] as { command?: string } | undefined;
